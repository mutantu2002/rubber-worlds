...
public class Main
{
	...
	{
		//new Thread(new WorldRunner(WorldFactory.createOneSquareObjectWorld(200,50,15,15), new WorldFrame(Constants.WIDTH,Constants.HEIGHT))).start();
		...
		//new Thread(new WorldRunner(WorldFactory.createOneParallelogramObjectWorld(200,50,5,10, 8), new WorldFrame(Constants.WIDTH,Constants.HEIGHT))).start();
		//new Thread(new WorldRunner(WorldFactory.createOneRoundObjectWorld(400,300,7,15), new WorldFrame(Constants.WIDTH,Constants.HEIGHT))).start();
		//new Thread(new WorldRunner(WorldFactory.create3ObjectsWorld(100,100,8,20), new WorldFrame(Constants.VIEWPORT_WIDTH,Constants.VIEWPORT_HEIGHT))).start();
		new Thread(new WorldRunner(WorldFactory.createFromFile("tempdata.ser"), new WorldFrame(Constants.VIEWPORT_WIDTH,Constants.VIEWPORT_HEIGHT))).start();
		...
		//new Thread(new WorldRunner(WorldFactory.createWorldFromImage(0, 0, 7), new WorldFrame(Constants.WIDTH,Constants.HEIGHT))).start();
	}
}
